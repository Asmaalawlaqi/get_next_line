...
char	*read_line(char *str, int fd)
{
	...
	buffer = malloc((long)BUFFER_SIZE + 1); //It enters a loop that reads from the file descriptor (fd) into the buffer
	if (!buffer)
		return (NULL);
	chars_read = 1; // by default we would always have one character to read because of NULL
	while (!ft_strchr(str, '\n') && chars_read != 0)
	{
		chars_read = read(fd, buffer, BUFFER_SIZE); //whatever is in fd, put in buffer and the BUFFER_SIZE is how many characters we read
		if (chars_read < -1)
		{
			free(str);
			free(buffer);
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || BUFFER_SIZE > 2147483647) // is less than or equal to 0, or
	//if BUFFER_SIZE is greater than the maximum allowed value
		return (0);
	str = read_line(str, fd); //calls a function read_line with the current value of str fd. The result is then assigned to the str variable.
	if (!str)
		return (NULL);
